...
echo "merged bam file: ${bam}, metrics file : ${metrics}"
echo "genome ctat dir: ${genome_ctat_lib}"

{
  ...
  python /usr/local/setup/nextflow/bin/JR_METe14EGFRVIII.py -i $sample_name -n \${totalreads} -f ${bam} -a ${resources}/METe14EGFRVarIII_hg19_anno.csv -e MET_E14_Skipping
  ev=\$?
